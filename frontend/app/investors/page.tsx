    ...
    <div className="min-h-screen bg-[#04070f] text-slate-200">
      ...
      <div className="px-4 md:px-12 py-10">
        <div className="max-w-5xl mx-auto">
          ...
          {showForm && (
            <div className="rounded-2xl border border-white/[0.08] p-6 mb-6"
              ...
              <div className="flex gap-2.5 mt-4">
                ...
                <button onClick={() => { setShowForm(false); setForm(EMPTY_FORM) }}
                  className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-medium text-slate-400 border border-white/[0.08] hover:bg-white/5 cursor-pointer"
                    ...
                    <>
                      ...
                      <div className="flex items-center gap-1.5">
                        <button onClick={() => handleSaveEdit(inv.id)} disabled={saving}
                          className="flex items-center justify-center w-8 h-8 rounded-lg text-emerald-400 border border-emerald-500/20 hover:bg-emerald-500/10 cursor-pointer"
                          style={{ background: "rgba(16,185,129,0.06)" }} title="Save">
                          <RiCheckLine size={15} />
                        </button>
                        <button onClick={cancelEdit}
                          className="flex items-center justify-center w-8 h-8 rounded-lg text-slate-500 border border-white/[0.08] hover:bg-white/5 cursor-pointer"
                          style={{ background: "transparent" }} title="Cancel">
                          <RiCloseLine size={15} />
                        </button>
                      </div>
                    </>
                    ...
                    <>
                      ...
                      <div>
                        <span className="text-[11px] px-2.5 py-1 rounded-full font-medium"
                          style={{ background: STATUS_STYLES[inv.status]?.bg || "rgba(107,114,128,0.12)", color: STATUS_STYLES[inv.status]?.color || "#9ca3af", border: `1px solid ${STATUS_STYLES[inv.status]?.border || "rgba(107,114,128,0.25)"}` }}>
                      ...
                      <div className="flex items-center gap-1.5">
                        <button onClick={() => startEdit(inv)}
                          className="flex items-center justify-center w-8 h-8 rounded-lg text-slate-500 border border-white/[0.08] hover:text-sky-400 hover:border-sky-500/20 hover:bg-sky-500/5 cursor-pointer"
                          style={{ background: "transparent" }} title="Edit">
                          <RiEditLine size={14} />
                        </button>
                        <button onClick={() => handleDelete(inv.id)}
                          className="flex items-center justify-center w-8 h-8 rounded-lg text-slate-500 border border-white/[0.08] hover:text-red-400 hover:border-red-500/20 hover:bg-red-500/5 cursor-pointer"
                          style={{ background: "transparent" }} title="Delete">
